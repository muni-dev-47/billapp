...
import { addBillItem, deleteBillItem, editBillItems, removeBillItems, setBillFormData, setEditFormData, setEditItems, updateBillItem } from '../redux/billSlice';
import { updateCustomerBill, updateSales } from '../redux/customerSlice';

const EditBill = () => {
    const location = useLocation();
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const { cusId, billId, date } = location.state;
    const bill = useSelector(store => store.bill);
    const editFormData = bill?.editFormData || {};
    const customerDetails = useSelector(store => store.customer.customerDetails)?.find(val => val.id === cusId) || []
    const customerBill = useSelector(store => store.customer.customerBills)?.find(val => val.id === cusId)?.bills || [];
    const billDetail = customerBill?.find(val => val.invoiceId === billId) || {};
    let billItems = bill?.billItems || []
    const billData = bill?.billInputForm;
    const customerName = customerDetails?.name;
    const customerContact = "+91 " + customerDetails?.mobile;
    const id = cusId
                ...
                <div className="card-body p-4">
                        ...
                        <div className="col-12 col-md-6">
                            <div className="border p-3 rounded bg-light">
                                ...
                                <div>
                                    ...
                                    <div>{billDetail?.date
                                        ? new Date(billDetail.date).toLocaleDateString("en-GB")
                                        : ""}</div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="d-flex flex-column flex-md-row justify-content-between gap-3 mt-4 pt-2 border-top">
                        ...
                        <div className="d-flex gap-2 order-1 order-md-2">
                            <button className="btn btn-warning" onClick={() => handleEditSales()}>
                                <i className="bi bi-pencil-square me-1"></i>Update Bill
                            </button>
                        </div>
                    </div>
                </div>
